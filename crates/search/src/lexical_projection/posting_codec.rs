...
fn pack(values: &[u32; BLOCK_LEN], output: &mut Vec<u8>) -> u8 {
    ...
    // The dependency stores native-endian words. The wire uses LE words.
...
pub(super) fn encode(postings: &[Posting]) -> Result<Vec<u8>> {
    ...
    let mode = if postings.len() < BLOCK_LEN {
        1
    } else if wide {
        2
    } else {
    ...
    bytes[8] = mode;
    bytes[12..20].copy_from_slice(&postings[0].ordinal.to_le_bytes());
...
mod tests {
    ...
    // Independent bit-at-a-time oracle for the four interleaved LE lanes.
