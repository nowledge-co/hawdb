...
mod tests {
    ...
    #[test]
    fn search_candidate_shadow_evidence_command_rejects_invalid_args_and_redacts_read_errors() {
        for args in [
            vec![],
            vec!["--unknown"],
            vec!["first", "second"],
        ...
        let path = unique_test_file("search_candidate_private_probe_path");
        let error = run_nowledge_search_candidate_shadow_evidence(
            [path.to_str().unwrap().to_string()].into_iter(),
        )
        .unwrap_err()
        .to_string();

        assert!(error.contains("failed to read search candidate shadow probe JSON"));
        assert!(!error.contains("search_candidate_private_probe_path"));
