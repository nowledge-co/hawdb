    ...
    let (
        ...
        Some(ActiveManifestUpdate::Replace {
            ...
            let document_count = active
                ...
                .ok_or_else(|| {
                    HawDBError::Storage("search generation update document count overflows".into())
                })?;
        ...
        Some(ActiveManifestUpdate::Compact {
            ...
            let start = active
                ...
                .ok_or_else(|| {
                    HawDBError::Storage(
                        "search segment compaction first source segment is no longer active".into(),
                    )
                })?;
            ...
            {
                return Err(HawDBError::Storage(
                    "search segment compaction selection is no longer active and contiguous".into(),
                ));
            }
            let selected = &active.segments[start..end];
            let source_level = selected[0].level;
            if (*target_level != source_level && *target_level != source_level.saturating_add(1))
            ...
            let selected_document_count = selected.iter().try_fold(0usize, |total, segment| {
                total.checked_add(segment.document_count).ok_or_else(|| {
                    HawDBError::Storage("search segment compaction document count overflows".into())
                })
            })?;
        ...
        Some(ActiveManifestUpdate::ReplaceRange {
            ...
            if *segment_count < 2 {
                return Err(HawDBError::Storage(
                    "search segment range replacement requires at least two source segments".into(),
                ));
            }
            let active_bytes = read_bounded_file(
                &input.root.join(OUT_OF_CORE_MANIFEST_FILE),
