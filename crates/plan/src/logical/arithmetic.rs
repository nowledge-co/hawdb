...
impl ComposedReturnBinder<'_> {
    fn bind(&mut self, expression: &ReturnExpression) -> Result<(ProjectionExpression, String)> {
        match &expression.kind {
            ...
            ReturnExpressionKind::Path(_) => Err(SkeinError::Semantic(
...
mod tests {
    ...
    fn bind(query: &str) -> PlannedReturns {
        let pipeline = skein_cypher::parse_pipeline(query).unwrap();
        let skein_cypher::ClauseKind::Return(projection) = &pipeline.clauses.last().unwrap().kind
        else {
