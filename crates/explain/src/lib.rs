//! Rendering for physical query plans and execution profiles.

use skein_executor::ReadExecutionProfile;
...
/// Writes a stable table representation of an optimized physical plan.
pub fn write_explain(
    formatter: &mut impl Write,
    physical_plan: &PhysicalPlan,
    trace: &OptimizerTrace,
    plan_cache_lookup: &str,
) -> fmt::Result {
    let rows = plan_rows(physical_plan, |node, operator_id, _is_root| ExplainRow {
        id: String::new(),
        estimated_rows: estimated_rows(trace, operator_id, node.kind()),
        actual_rows: None,
        task: "root".to_string(),
    ...
    write!(
        ...
        trace.search_mode.as_str(),
        trace.groups,
        trace.selected_plan_cost.cost,
        plan_cache_lookup,
        trace.query_digest.as_deref().unwrap_or(NOT_AVAILABLE),
        trace.selected_plan_fingerprint,
    )
}

/// Writes a stable table representation of an analyzed physical plan.
pub fn write_explain_analyze<T>(
    formatter: &mut impl Write,
    physical_plan: &PhysicalPlan,
    trace: &OptimizerTrace,
    plan_cache_lookup: &str,
    execution_profile: &ReadExecutionProfile<T>,
) -> fmt::Result {
    let rows = plan_rows(physical_plan, |node, operator_id, is_root| {
        let blocking = execution_profile
        ...
        ExplainRow {
            id: String::new(),
            estimated_rows: estimated_rows(trace, operator_id, node.kind()),
            actual_rows: actual_rows(execution_profile, operator_id, node.kind()),
            task: "root".to_string(),
            access_object: access_object(node),
            execution_info: if is_root {
                Some(root_execution_info(execution_profile))
            } else {
            ...
            memory: blocking
                ...
                .or_else(|| {
                    is_root.then(|| {
                        let pipeline = &execution_profile.pipeline_memory_report;
                        format!(
                            "peak={}/budget={}",
...
fn actual_rows<T>(
    profile: &ReadExecutionProfile<T>,
    operator_id: PhysicalOperatorId,
    operator: PhysicalPlanKind,
) -> Option<String> {
    profile
...
fn root_execution_info<T>(profile: &ReadExecutionProfile<T>) -> String {
...
fn write_table(formatter: &mut impl Write, headers: &[&str], rows: &[Vec<&str>]) -> fmt::Result {
    let widths = headers
        .iter()
        .enumerate()
        .map(|(column, header)| {
...
fn write_border(formatter: &mut impl Write, widths: &[usize]) -> fmt::Result {
