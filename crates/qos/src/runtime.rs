...
impl RuntimePermit {
    ...
    /// Binds the resources represented by this permit to an execution context.
    pub fn execution_context(&self, parent: &RuntimeTaskContext) -> RuntimeTaskContext {
        let permit_reservation = skein_core::RuntimeMemoryReservation::new(
            self.request.memory_bytes,
            self.request.result_bytes,
        );
        let execution_reservation = parent
            .memory_reservation()
...
mod tests {
    ...
    #[test]
    fn permit_binds_its_exact_resources_to_the_execution_context() {
        let governor = governor(2, 4 * 1024 * 1024 * 1024);
        let request = RuntimeWorkRequest::foreground_query(128, 32).with_cpu_slots(2);
        let permit = governor.try_admit(request).unwrap();

        let context = permit.execution_context(&RuntimeTaskContext::default());
        ...
        let bounded = permit.execution_context(&parent);
