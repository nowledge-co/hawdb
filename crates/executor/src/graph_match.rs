...
use skein_storage::{AdjacencyDirection, NodeId, NodeRecord, RelId};
            ...
            match import.kind {
                GraphEntityKind::Node => {
                    let node = self.store.node_owned(NodeId(id))?.ok_or_else(|| {
                        SkeinError::Execution("bound node disappeared during MATCH".to_string())
                    })?;
                    leases.push(
            ...
            GraphMatchStep::Expand {
                ...
                if *min_hops != 1 || *max_hops != 1 {
                    if relationship.is_some()
                        || !properties.is_empty()
                        || *direction != skein_core::RelationshipDirection::Outgoing
                        || rel_type_id.is_none()
                    {
                        return Err(SkeinError::Execution("bounded MATCH expansion requires an outgoing typed pattern without relationship bindings".to_string()));
                    }
