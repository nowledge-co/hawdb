...
mod tests {
    ...
    fn spill_round_trip_preserves_bindings() {
        let uuid = Value::Uuid(hawdb_core::Uuid::from_bytes([0xa5; 16]));
        let binding = Binding {
    ...
    fn uuid_spill_preflight_checks_every_truncation_and_preserves_alignment() {
        for fill in [0, 8, 0x80, 0xff] {
            let value = Value::Uuid(hawdb_core::Uuid::from_bytes([fill; 16]));
    ...
    #[test]
    fn pool_startup_removes_only_eligible_hawdb_orphans() {
        let memory = test_memory("orphan-cleanup");
        std::fs::create_dir_all(&memory.spill_directory).unwrap();
        let orphan = memory
            .spill_directory
            .join("hawdb-spill-v1-stale-process-sort-1.spill");
