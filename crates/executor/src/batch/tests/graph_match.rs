...
fn execute(query: &str) -> Result<Vec<Binding>> {
    let logical = skein_plan::plan_pipeline_query(query, &BTreeMap::new())?;
...
fn scalar_property_predicates_do_not_read_shadowed_nodes() {
    ...
    let plan = lower(
        skein_plan::plan_pipeline_query(
...
fn match_propagates_errors_cancellation_and_downstream_stop() {
    ...
    let plan = lower(
        skein_plan::plan_pipeline_query("MATCH (n:Memory) RETURN n", &BTreeMap::new()).unwrap(),
    );
        ...
        for fail in [false, true] {
            ...
            let result =
                execute_binding_batches(&plan, context, ExecutionLimit::unlimited(), &mut |_| {
                    calls += 1;
                    if fail {
                        Err(SkeinError::Execution("consumer failure".into()))
                    } else {
...
fn visibility_filters_inside_optional_matching_and_survives_with() {
    ...
    let logical = skein_plan::plan_pipeline_query(query, &BTreeMap::new()).unwrap();
    let visible =
        skein_plan::apply_node_visibility_predicates(logical, &|variable| Predicate::PropertyIn {
...
fn match_expansion_checks_cancellation_before_rejecting_target_labels() {
    ...
    use skein_storage::{AdjacencyDirection, RelRecord};
    with_context(None, |context| {
        let token = skein_core::RuntimeCancellationToken::new();
    ...
    with_context(None, |context| {
        ...
        let store = super::store::ReadFixture {
            nodes: (0..6)
                ...
                .collect(),
            relationships: (1..6)
                .map(|id| skein_storage::RelRecord {
                    id: skein_storage::RelId(id),
        ...
        for (distinct, expected) in [("", 12), ("DISTINCT ", 5)] {
            ...
            let plan = lower(skein_plan::plan_pipeline_query(&query, &BTreeMap::new()).unwrap());
