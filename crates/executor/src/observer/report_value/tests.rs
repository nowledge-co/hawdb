...
fn blocking_fixture(data: &mut Data) -> (BlockingOperatorMemoryReport, Value) {
    let report = BlockingOperatorMemoryReport {
        ...
        replay_rows: data.size(),
        repartitions: data.size(),
    ...
    let expected = object([
        ...
        ("candidate_rows", number(report.candidate_rows as u128)),
        ("replay_rows", number(report.replay_rows as u128)),
        ("repartitions", number(report.repartitions as u128)),
