...
use super::*;
use hawdb_core::{IndexKind, PropertyType, TableKind};
...
impl TestRoot {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "hawdb-statistics-kernel-{}-{stamp}-{id}",
            std::process::id()
        ));
...
fn run_admission_preserves_inclusive_fact_memory_spill_byte_and_run_limits() {
    ...
    for bytes in [8, 9] {
        ...
        if bytes == 8 {
            ...
            assert_eq!(
                fs::read(spill.path().join("run.00000000.hawdb")).unwrap(),
                b"pc\t0\t0\t1\n"
            );
        }
        drop(spill);
...
fn corrupt_missing_and_existing_runs_fail_and_transient_files_are_reclaimed() {
    ...
    }
    {
        let spill = RefreshSpillDirectory::create(&root.0).unwrap();
        let mut writer = StatsRunWriter::new(spill.path(), &catalog, &options);
        let path = spill.path().join("run.00000000.hawdb");
