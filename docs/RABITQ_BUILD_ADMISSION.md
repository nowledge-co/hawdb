...
Exact/one-short RaBitQ phase trials keep the real build options, startup paths
and constructed builder charged. A competing reservation pads that combined
occupancy to a fixed 256 KiB test envelope (after asserting construction fits).
Constructor admission is tested independently. Stage sequence digits and sandbox-root
...
The wrapper's path and original name owners are detailed in
`ARTIFACT_PATH_ADMISSION.md`; backend-internal path copies remain separate.
