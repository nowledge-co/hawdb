# HawDB Storage TLA+ Models

These models specify storage publication and recovery protocols implemented by
the embedded HawDB library or required before a planned storage path may be
...
`HawDBCowPagePublication` uses TLC's `-lncheck final` in both Bazel and the
...
`hawdb_storage::durability::WalSyncGroupState` owns the accumulated record and
...
`HawDBWalGroupCommit.tla` models the bounded request queue and the leader-owned
...
`HawDBWalDoctor.tla` models the destructive repair protocol separately from
...
`HawDBAppendTable.tla` models the generic admission and visibility contract for
...
`HawDBAppendSegmentPublication.tla` models checkpointing durable append batches
...
`HawDBAppendMixedTransaction.tla` models a single transaction whose private
...
`HawDBGeneratedAppendOrder.tla` models table-wide `commit_sequence`
...
`HawDBKnowledgeRetrievalPipeline.tla` models the application-facing retrieval
...
`HawDBQueryMemoryLedger.tla` models the runtime refinement implemented by
...
their visibility follows `HawDBConcurrentSnapshots.tla`; durable canonical
adjacency ordering and publication continue to follow
`HawDBGraphDescriptorPaging.tla`.
...
`HawDBBoundedMorselMerge.tla` models the production shared-pool result path for
...
`HawDBGenerationReclamation.tla` models immutable checkpoint generations and the
...
`HawDBProjectionChangefeed.tla` models the unified graph and relational
...
`HawDBCowPagePublication.tla` specifies the publication protocol required by
...
unselected; `HAWDB_MANIFEST_V1` then binds the exact row and overflow
...
composition of `HawDBCowPagePublication.tla`, `HawDBTransactionConcurrency.tla`,
`HawDBTransactionIndexOverlay.tla`, `HawDBStorageDurability.tla`, and
`HawDBWalGroupCommit.tla`.
...
`HawDBContentSourceReplacement.tla` models the Content Store operation that
...
`HawDBContentSourceOwnershipMove.tla` models the Content Store operation that
...
`HawDBContentThreadOwnershipMove.tla` models a bounded batch of Thread
...
`HawDBContentSpaceMergeOwnership.tla` models the cross-kind ownership batch
...
`HawDBContentThreadUpsert.tla` models the initial Thread content write as one
...
`HawDBContentThreadReconcile.tla` models occurrence-preserving Thread message
...
`HawDBContentThreadTailDelete.tla` models deletion of an exact ordered Thread
...
`HawDBContentThreadDelete.tla` models whole-Thread deletion across the graph
...
`HawDBOverflowPublication.tla` models the generation-bound overflow root used
...
`HawDBOverflowExactCompaction.tla` models the separately admitted metadata-only
...
`HawDBRowPageMutation.tla` models a persistent table-scoped allocator and the
...
`HawDBRowRecovery.tla` models the runtime integration between one
...
`HawDBRelationalRowSnapshotRead.tla` and bound to the Rust SQL runtime below.

## Immutable Relational Row Live Views

`HawDBRowLiveView.tla` models publication after a complete recovery view has
...
`HawDBTransactionIndexOverlay.tla` models one multi-statement authoritative SQL
...
`HawDBIndexPublication.tla` refinement obligations.

## Immutable Relational Row-Delta Runs

`HawDBRowDeltaRuns.tla` models the non-serving disk-backed generation written
...
and corruption rejection outside the abstract model. `HawDBRowRecovery.tla`
separately proves how complete WAL replay selects that artifact before the live
view may advance.

## Relational Row Snapshot Composition

`HawDBRelationalRowSnapshotRead.tla` models the logical newest-version overlay,
...
`HawDBRelationalOverlayStreamingMerge.tla` owns the physical refinement: one
...
`HawDBSparseRelationalRecovery.tla` composes the source-fenced row and index
...
`HawDBSparseWritableRelationalRecovery.tla` models the per-record writable
...
`HawDBSparseRelationalLiveStage.tla` models schema-stable live DML after a
...
`HawDBSparseRelationalLivePreparation.tla` models the closure loop before live
...
`HawDBSparseRelationalLiveCommit.tla` models the `GraphStore` boundary after
...
`HawDBSparseRelationalActivation.tla` models the `OutOfCore` plus
...
`HawDBRelationalWalReplayAccess.tla` models the bounded primary-key working set
...
`HawDBProjectionDurability.tla` models the durable projection framework of
...
`HawDBProjectionGenerationReplacement.tla` models the publish-last owner head
defined by
...
`HawDBCompactionVisibility.tla` models a derived column-group read path
governed by
[`../specs/ROW_PAGE_AND_DEMAND_PAGED_INDEX_SPEC.md`](../specs/ROW_PAGE_AND_DEMAND_PAGED_INDEX_SPEC.md):
...
`HawDBGenerationReclamation.tla`.
...
`HawDBColumnGroupManifest.tla` models the publication protocol in
[`../specs/ROW_PAGE_AND_DEMAND_PAGED_INDEX_SPEC.md`](../specs/ROW_PAGE_AND_DEMAND_PAGED_INDEX_SPEC.md).
...
`HawDBRuntimeAdmission.tla` models the governor's split between memory
...
existing `HawDBRuntimeAdmission.tla` safety and liveness obligations apply to
both derived configurations independently.

`HawDBMemoryTierRelease.tla` models the separate release-evidence boundary. A
...
`HawDBColumnarShadowIntegration.tla` models the shadow adoption phase of
[`../specs/ROW_PAGE_AND_DEMAND_PAGED_INDEX_SPEC.md`](../specs/ROW_PAGE_AND_DEMAND_PAGED_INDEX_SPEC.md)
...
admission semantics are modeled separately by `HawDBRuntimeAdmission.tla`
below.
...
| Checkpoint publication selects one complete graph, row-page, and overflow generation; generation-manifest replace failure and other unbound candidates cannot replace it, while corrupt bound generation metadata fails open closed | checkpoint failpoints, `durable_replace_file`, `RelationalRowPagePublisher::persist_generation`, `RelationalOverflowPublisher::persist_generation`, and `HAWDB_MANIFEST_V1` replacement | `checkpoint_publish_failpoints_recover_one_complete_generation`, `generation_manifest_replace_failure_leaves_the_canonical_row_root_unselected`, `generation_manifest_replace_failure_leaves_the_canonical_overflow_root_unselected`, `platform_obstruction_rejects_canonical_row_and_overflow_manifest_replace`, `canonical_checkpoint_rewrites_only_dirty_relational_pages`, `unbound_row_candidate_does_not_replace_canonical_recovery`, `corrupt_bound_row_root_fails_database_open`, `subprocess_crash_matrix_recovers_whole_batches_and_artifact_generations`, `HawDBCowPagePublication.tla` |
...
| HawDB Lightning derives a registry-complete export without advancing an in-memory source epoch and imports only a valid stream into an empty or verified engine-only target | `Database::hawdb_lightning_relational_state`, `Database::hawdb_lightning_initial_import_apply_internal`, `GraphStore::import_hawdb_snapshot_rows_with_source_fingerprint` | `hawdb_lightning_initial_import_apply_imports_database_state_into_empty_target`, `hawdb_lightning_initial_import_rejects_stream_without_engine_registry` |

## In-memory Snapshot Publication

`HawDBConcurrentSnapshots.tla` models `hawdb-storage::SnapshotCoordinator`.
...
`HawDBCompositePropertyProjection.tla` makes the composite serving refinement
...
`HawDBRelationshipPropertyProjection.tla` adds the one-hop relationship
...
`HawDBGraphIndexQualification.tla` models the release-selection gate shared by
...
`hawdb-graph-index-qualification` plan parser only constructs this existing
typed all-class matrix with one caller-owned read-only path and has no
independent durable or activation transition.
The bounded `hawdb-graph-storage-qualification` parser likewise constructs one
...
`HawDBRelationalIndexShadowPublication.tla` models the generation-aligned but
...
authoritative actions in `HawDBIndexPublication.tla` and implemented only when
...
`HawDBRelationalIndexDemandRead.tla` models the generation-pinned demand-reader
...
`HawDBRelationalRowDemandRead.tla` models the canonical row-slot reader
...
`HawDBIndexRecovery.tla` models recovery and live-delta mechanics independently
...
`HawDBPageCacheAdmission.tla` models the clean immutable page-cache boundary
...
`HawDBVerifiedRowPageCache.tla` refines the fixed-slot relational row-page
...
`HawDBStableIdentityPublication.tla` models the independent physical-id to
stable-identity sidecar used by HawDB Lightning. Candidate fixed-size pages
...
`HawDBStatisticsEligibility.tla` models the property-statistics type boundary.
...
`HawDBExactGraphCounts.tla` models the basic Count Store as part of canonical
...
durability and snapshot publication remain owned by `HawDBStorageDurability`
and `HawDBConcurrentSnapshots`.

`HawDBIndexStatistics.tla` models the payload-free sample maintained for each
...
`HawDBSystemSchemaUpgrade.tla` models startup validation, ordered migration
staging, the shared WAL durability decision, handle publication, DDL failure,
read-only rejection, crash recovery, and the HawDB Lightning registry boundary.
...
## CRDT Replication Between HawDB Nodes

`HawDBCrdtReplication.tla` models the delta-state CRDT contract in
[`../specs/HAWDB_CRDT_REPLICATION_SPEC.md`](../specs/HAWDB_CRDT_REPLICATION_SPEC.md).
...
`HawDBGossipDelivery.tla` models the delivery layer of the master-slave CRDT
contract. It is deliberately separate from `HawDBCrdtReplication.tla`: that
...
sed -e 's/^SPECIFICATION FairSpec/SPECIFICATION SlaveFairSpec/' \
    ...
    docs/tla/HawDBGossipDelivery.cfg > /tmp/partition.cfg
```
