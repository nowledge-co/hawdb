...
The initial application schema is Nowledge Content Store. Its owner is
`nowledge.content_store`, and its version 1 migration creates
`content_documents`, `thread_messages`, `content_chunks`, `content_anchors`,
`content_migration_state`, and their indexes.
