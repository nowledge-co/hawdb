...
unchanged execution budgets for both layouts. The term limit is a required
argument and is passed through the public policy-aware writer and reader. It
does not change the 4096-byte product default or waive any independent resource
limit. Source/term normalization, sampling, truncation, and skipped records are
not permitted to make a measurement pass.

```sh
cargo test --locked --example lexical_corpus_qualification
cargo build --locked --release --example lexical_corpus_qualification
./target/release/examples/lexical_corpus_qualification /path/to/thread_messages.jsonl /path/to/new-generation 334844 1048576
...
The illustrative 1 MiB term policy above is host-selected for measurement, not a
new default. All remaining build options are default values and are included in
the report alongside the selected term policy and analyzer digest.
