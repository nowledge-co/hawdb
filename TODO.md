...
- [ ] Connect the bounded PostgreSQL-dialect frontend to the production data
  plane.
  - The owned AST, dense `$n` prepared parameters, and corpus-required
    SELECT/DDL/DML grammar are specified and covered by the relational Content
    Store specification.
  - Reject unsupported PostgreSQL syntax during binding with a stable error;
    never accept syntax and silently change its semantics.
  - Acceptance: the existing `system.*` queries and every statement-corpus
    fixture lower through the same parser, binder, logical plan, and executor
    entrypoints.

- [ ] Integrate first-class relational tables into the canonical durable store.
  - The separate relational catalog, scalar types, paged COW rows/indexes,
    final-state constraints, overflow values, WAL batches, and checkpoints are
    specified and implemented as storage primitives.
  - Extend WAL, checkpoint, replay, manifest integrity, and doctor inspection
    so one canonical commit atomically covers graph and relational mutations;
    do not expose a second sidecar WAL as a production transaction boundary.
  - Acceptance: crash injection cannot publish a partial multi-table mutation,
    a uniqueness violation, or an anchor that references a non-visible row.

- [ ] Move bounded overflow segments from the in-memory qualification store to
  the canonical file-backed storage path.
  - The 4 KiB threshold, raw/Zstd selection, checksummed envelope, late
    hydration budgets, snapshot pinning, checkpoint codec, and reachability GC
    are specified and implemented as relational storage primitives.
  - Publish overflow segments as immutable file-backed pages and include them
    in the canonical manifest, backup, restore, scrub, doctor, and reclamation
    protocols.
  - Acceptance: a content query cannot require all message or chunk bodies to
    become resident, and malformed or oversized compressed data fails before
    unbounded allocation.

- [ ] Execute relational SQL through the existing optimizer and batch pipeline.
  - Completed foundation: qualified conjunctive joins now derive right-side
    composite primary-key or leading index-prefix access through Skyline;
    prefix postings are visited without full collection and selected join paths
    are retained for diagnostics.
  - Completed foundation: aggregate groups use incremental state rather than
    retaining every raw binding, and dynamic group/distinct state is admitted
    through the shared blocking-operator memory tracker. Input batching,
    aggregate spill, cancellation, and public query-report wiring remain open.
  - Add relational table/index scans, key/range lookup, projection, filter,
    join, aggregate, sort, distinct, offset, and limit plans without a
    content-route-specific executor branch.
  - Push predicates and limits below large-value hydration and expose fast-path
    selection in `EXPLAIN`/`EXPLAIN ANALYZE` and query reports.
  - Reuse the blocking-operator memory tracker and spill contracts for SQL
    joins, aggregates, distinct, and sort; propagate cancellation through scan,
    spill, and overflow-value reads.
  - Extend the existing embedded transaction/session entrypoints to stage
    named parameterized SQL together with Cypher mutations. Do not add one
    typed Skein API per content route.
  - Acceptance: foreground admission bounds workers, intermediate rows, spill,
    payload bytes, and decompressed bytes for every statement in the corpus.
