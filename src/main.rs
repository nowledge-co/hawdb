        ...
        if command == "nowledge-replacement-summary" {
            ...
            while let Some(flag) = args.peek() {
                match flag.as_str() {
                    ...
                    "--explore-parity-evidence-json" => {
                        args.next();
                        explore_parity_evidence_path = Some(args.next().ok_or_else(|| {
                            SkeinError::Semantic(nowledge_replacement_summary_usage())
                        })?);
                    }
                    "--expand-parity-evidence-json" => {
                        args.next();
                        expand_parity_evidence_path = Some(args.next().ok_or_else(|| {
                            SkeinError::Semantic(nowledge_replacement_summary_usage())
                        })?);
                    }
                    ...
                    "--live-preview-node-parity-evidence-json" => {
                        args.next();
                        live_preview_node_parity_evidence_path =
                            Some(args.next().ok_or_else(|| {
                                SkeinError::Semantic(nowledge_replacement_summary_usage())
                            })?);
                    }
                    "--node-details-parity-evidence-json" => {
                        args.next();
                        node_details_parity_evidence_path = Some(args.next().ok_or_else(|| {
                            SkeinError::Semantic(nowledge_replacement_summary_usage())
                        })?);
                    }
                    ...
                    "--community-members-parity-evidence-json" => {
                        args.next();
                        community_members_parity_evidence_path =
                            Some(args.next().ok_or_else(|| {
                                SkeinError::Semantic(nowledge_replacement_summary_usage())
                            })?);
                    }
                    "--community-subgraph-parity-evidence-json" => {
                        args.next();
                        community_subgraph_parity_evidence_path =
                            Some(args.next().ok_or_else(|| {
                                SkeinError::Semantic(nowledge_replacement_summary_usage())
                            })?);
                    }
                    "--community-recent-memories-parity-evidence-json" => {
                        args.next();
                        community_recent_memories_parity_evidence_path =
                            Some(args.next().ok_or_else(|| {
                                SkeinError::Semantic(nowledge_replacement_summary_usage())
                            })?);
                    }
...
mod tests {
    ...
    fn merges_replacement_summary_evidence_artifacts() {
        ...
        let expand_path = unique_json_file("expand_parity_evidence");
        ...
        let community_members_path = unique_json_file("community_members_parity_evidence");
        ...
        .unwrap();
        std::fs::write(
            &community_recent_memories_path,
            serde_json::json!({
                "protocol": "nmem-graph-route-shadow-parity-evidence-v1",
                "ready": true,
        ...
        assert_eq!(
            bundle["expand_parity_evidence"]["route"],
        ...
        assert_eq!(
            bundle["live_preview_node_parity_evidence"]["route"],
            "/graph/live-preview/{node_id}"
        );
        assert_eq!(
            bundle["node_details_parity_evidence"]["route"],
        ...
        assert_eq!(
            bundle["community_members_parity_evidence"]["route"],
            "/graph/community-members/{community_id}"
        );
        assert_eq!(
            bundle["community_subgraph_parity_evidence"]["route"],
            "/library/community/{community_id}/subgraph"
        );
        assert_eq!(
            bundle["community_recent_memories_parity_evidence"]["route"],
        ...
        std::fs::remove_file(expand_path).unwrap();
        std::fs::remove_file(live_preview_path).unwrap();
        std::fs::remove_file(live_preview_node_path).unwrap();
        std::fs::remove_file(node_details_path).unwrap();
        std::fs::remove_file(orphans_path).unwrap();
        std::fs::remove_file(shortest_path_path).unwrap();
