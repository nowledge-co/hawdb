use super::{bind_relational_value, compile_column, reject_non_public_schema};
...
pub(crate) struct AppendSelectPlan {
    pub(crate) table: String,
    pub(crate) partition: skein_storage::RelationalKey,
    pub(crate) after: Option<skein_storage::RelationalKey>,
    pub(crate) max_rows: usize,
    projection: Vec<AppendProjection>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct AppendExplainPlan {
    pub(crate) select: AppendSelectPlan,
...
pub(crate) fn compile_append_statement_sql(
    sql: &str,
    parameters: &[Value],
    state: &AppendState,
    ...
    match prepared.statement {
        ...
        SqlStatement::Update(update) if state.schema(&update.table.name).is_some() => Err(
            SkeinError::Semantic("strict append tables do not support UPDATE".to_string()),
        ),
        SqlStatement::Delete(delete) if state.schema(&delete.table.name).is_some() => Err(
            SkeinError::Semantic("strict append tables do not support DELETE".to_string()),
        ),
        SqlStatement::CreateIndex(create) if state.schema(&create.table.name).is_some() => Err(
            SkeinError::Semantic("strict append tables do not support CREATE INDEX".to_string()),
        ),
...
fn compile_insert_values(
    ...
    for column in insert_columns {
        let position = columns
            ...
            .ok_or_else(|| {
                SkeinError::Semantic(format!("table {table_name} has no column {column}"))
            })?;
...
pub(crate) fn compile_append_select_sql(
    sql: &str,
    parameters: &[Value],
    state: &AppendState,
...
pub(crate) fn compile_append_explain_sql(
    sql: &str,
    parameters: &[Value],
    state: &AppendState,
    configured_max_rows: usize,
...
fn compile_append_select(
    ...
    let requested = bind_append_bound(select.limit, parameters, "LIMIT")?.ok_or_else(|| {
        SkeinError::Semantic("strict append SELECT requires an explicit LIMIT".to_string())
    })?;
    ...
    Ok(AppendSelectPlan {
        ...
        partition: skein_storage::RelationalKey(vec![partition]),
        after: after.map(|value| skein_storage::RelationalKey(vec![value])),
        max_rows,
        projection,
    })
}
...
pub(crate) fn project_append_rows(
...
pub(crate) fn format_append_explain(
    plan: &AppendExplainPlan,
    report: Option<&skein_storage::AppendSegmentReadReport>,
