...
use skein_storage::{
    RelationalHydrationBudget, RelationalKey, RelationalRow, RelationalState,
...
pub(crate) fn execute_relational_query_sql(
    ...
    let SqlStatement::Select(select) = prepared.statement else {
        return Err(SkeinError::Semantic(
            "relational query entrypoint requires SELECT".to_string(),
        ));
    };
    execute_select(select, parameters, state, limits)
}
...
fn execute_select(
    ...
    let mut intermediate_rows = 0usize;
    ...
    let mut rows = base_entries
        .into_iter()
        .map(|(key, row)| BoundRow {
        ...
        .collect::<Vec<_>>();
    account_intermediate(
        &mut intermediate_rows,
        rows.len(),
        ...
        for row in rows {
            ...
            visit_join_entries(
                ...
                &mut |key, candidate| {
                    ...
                    if predicate_truth(&join.on, &combined, parameters)? == Some(true) {
                        matched = true;
            ...
            if !matched && join.kind == SqlJoinKind::Left {
                ...
                joined.push(combined);
                account_intermediate(&mut intermediate_rows, 1, limits.max_intermediate_rows)?;
            }
        }
        rows = joined;
    }
    ...
    if let Some(selection) = &select.selection {
        ...
        for row in rows {
            if predicate_truth(selection, &row, parameters)? == Some(true) {
                selected.push(row);
            }
        }
        rows = selected;
    ...
    if !select.order_by.is_empty() {
        ...
        rows.sort_by(|left, right| {
            ...
            match compare_bound_rows(left, right, &select.order_by) {
    ...
    let selected = rows
        .into_iter()
        .skip(offset)
        .take(requested.min(limits.max_output_rows.saturating_add(1)))
        .collect::<Vec<_>>();
    if selected.len() > limits.max_output_rows {
    ...
    for row in selected {
        let projected = project_bound_row(&row, &select.projection, state, &mut hydration)?;
        if select.distinct && !distinct.insert(row_identity(&projected)) {
            continue;
        }
...
fn visit_join_entries<'a>(
    ...
    row: &BoundRow<'a>,
    access: &RelationalJoinAccess,
    visit: &mut dyn FnMut(&'a RelationalKey, &'a RelationalRow) -> Result<()>,
) -> Result<()> {
    match access {
...
fn base_entries<'a>(
    ...
    access: RelationalBaseAccess,
    max_entries: usize,
) -> Vec<(&'a RelationalKey, &'a RelationalRow)> {
    match access {
        RelationalBaseAccess::PrimaryKey(key) => state.row_entry(table, &key).into_iter().collect(),
        RelationalBaseAccess::Index { name, prefix } => state
            .index_prefix_lookup(table, &name, &prefix, max_entries)
            .into_iter()
            .flatten()
            .filter_map(|primary_key| state.row(table, primary_key).map(|row| (primary_key, row)))
            .collect(),
        RelationalBaseAccess::FullScan => state.rows(table).take(max_entries).collect(),
    }
}
...
fn compare_bound_rows(
    left: &BoundRow<'_>,
    right: &BoundRow<'_>,
    order: &[crate::sql::SqlOrderItem],
) -> Result<Ordering> {
    for item in order {
        let left = resolve_column(left, &item.column)?;
        let right = resolve_column(right, &item.column)?;
        if matches!(left, RelationalValue::Overflow(_))
            || matches!(right, RelationalValue::Overflow(_))
        {
            return Err(SkeinError::Execution(
        ...
        let nulls_first = match item.nulls {
            ...
            SqlNullOrder::DialectDefault => item.direction == SqlOrderDirection::Desc,
        };
        let (ordering, reverse) = match (
            matches!(left, RelationalValue::Null),
            matches!(right, RelationalValue::Null),
        ) {
            (true, true) => (Ordering::Equal, false),
            (true, false) => {
                if nulls_first {
                    (Ordering::Less, false)
                } else {
            ...
            (false, false) => (left.cmp(right), item.direction == SqlOrderDirection::Desc),
        };
...
fn projected_value(
    ...
    if let std::collections::btree_map::Entry::Vacant(entry) = hydrated.entry(binding_index) {
        ...
        let row = state
            .hydrate_row(&binding.table, key, hydration)
...
fn row_identity(row: &Row) -> Vec<(String, Value)> {
    row.iter()
