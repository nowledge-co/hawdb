...
fn reject_optimistic_locking_select(prepared: &skein_sql::PreparedPostgresStatement) -> Result<()> {
...
fn sql_lock_requests(
    sql_text: &str,
    prepared: &skein_sql::PreparedPostgresStatement,
    parameters: &[Value],
...
mod tests {
    ...
    fn append_explain_analyze_takes_a_shared_database_lock() {
        ...
        let prepared =
            skein_sql::prepare_postgres_sql(SQL).expect("prepare append explain analyze");
        let requests = sql_lock_requests(
            SQL,
            &prepared,
            &[],
