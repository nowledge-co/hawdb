use super::{Database, QueryOutput};
use crate::error::{Result, SkeinError};
use crate::value::Value;
use skein_integrity::IntegrityHasher;
use skein_storage::{RelationalState, RelationalWrite};
...
const REGISTRY_READ_SQL: &str = "SELECT version, name, checksum \
    FROM skein_schema_migrations WHERE owner = $1 ORDER BY version ASC";
...
fn read_applied_migrations(
    ...
    let QueryOutput { rows } = db.query_sql_with_params_bounded(
        REGISTRY_READ_SQL,
        &[Value::String(owner.to_string())],
        Some(max_rows),
    )?;
    rows.into_iter()
        .map(|row| applied_migration_from_row(owner, row))
        .collect()
}
...
fn applied_migration_from_row(
    ...
    let version = match row.remove("version") {
    ...
    let checksum = match row.remove("checksum") {
