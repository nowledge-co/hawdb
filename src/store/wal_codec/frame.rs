...
pub(crate) const WAL_BINARY_MAGIC: &[u8; 8] = b"SKWALB01";
pub(crate) const WAL_BLOCK_BYTES: usize = 32 * 1024;
pub(crate) const WAL_FRAGMENT_HEADER_BYTES: usize = 15;
pub(crate) const WAL_BINARY_FILE_HEADER_BYTES: usize = 28;
...
pub(crate) fn decode_binary_wal_header(bytes: &[u8]) -> Result<(u64, u64)> {
...
pub(crate) fn frame_binary_wal_record(generation: u64, payload: &[u8], position: u64) -> Vec<u8> {
...
#[derive(Debug)]
pub(crate) enum BinaryWalReadEvent {
    Record {
        payload: Vec<u8>,
        start_offset: u64,
        end_offset: u64,
...
pub(crate) struct BinaryWalReader<R: Read> {
    reader: R,
    generation: u64,
...
impl<R: Read> BinaryWalReader<R> {
    pub(crate) fn new(reader: R, generation: u64, max_record_bytes: Option<usize>) -> Self {
        Self {
            reader,
            generation,
            max_record_bytes,
    ...
    pub(crate) fn next_event(&mut self) -> Result<BinaryWalReadEvent> {
        if self.finished {
