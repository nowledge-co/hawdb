...
pub use skein_search::projection_evidence::{
...
mod tests {
    ...
    #[test]
    fn search_projection_evidence_facade_preserves_owner_type_identity() {
        let contract = crate::nowledge_search_projection_probe_contract_json();
        let probe = &contract["example_skein_probe"];
        let owner: skein_search::projection_evidence::NowledgeSearchProjectionEvidenceReport =
            crate::NowledgeSearchProjectionEvidenceReport::from_probe(probe);
        let facade: crate::NowledgeSearchProjectionEvidenceReport = owner;
        assert!(facade.ready);
        assert_eq!(
            facade.json(),
            nowledge_search_projection_evidence_json(probe)
