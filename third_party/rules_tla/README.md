...
copyright are retained in `LICENSE`. Skein owns the local changes below.
...
negative control does not prove a model valid. Skein's collector additionally
...
shared `bazel-bin` directory containing other shards' stale outputs. Skein's
